...
exports = module.exports = function(req, res) {
	...
	locals.section = 'me';
	//locals.page.title = 'Settings - MetUGo';
...
*/		
	...
	view.on('post', { action: 'profile.pic' }, function(next) {
		...
		req.user.getUpdateHandler(req).process(req.body, {
			...
			req.flash('success', 'Your profile picture have been saved.');
			return next();
	...
	view.on('post', { action: 'profile.details' }, function(next) {
		...
		req.user.getUpdateHandler(req).process(req.body, {
			fields: 'name, email,introduction',
			flashErrors: true
		}, function(err) {
